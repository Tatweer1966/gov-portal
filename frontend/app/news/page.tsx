...
export default function NewsPage() {
    ...
    async function loadNews() {
      try {
        ...
        if (!controller.signal.aborted) {
          console.error('Failed to load Directus news:', error);
          setNews([]);
        }
      } finally {
        if (!controller.signal.aborted) {
  ...
  return (
    <div
      dir="rtl"
      className="min-h-screen bg-gradient-to-b from-primary/5 to-background py-12"
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-10">
          ...
          <p className="text-muted-foreground">
            تابع المستجدات والبيانات الرسمية الصادرة عن المحافظة
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {news.map((item, idx) => (
              <motion.div
                ...
                whileHover={{ y: -5 }}
                className="bg-card rounded-2xl border border-border shadow-sm hover:shadow-xl transition-all h-full overflow-hidden"
              >
